...
if (isset($_POST['sumbit'])) {
    ...
    if ($start != null || $end != null) {
        ...
        SELECT date AS tanggal, username AS nama, amount AS jumlah, keterangan as Keterangan 
        FROM transactions 
        JOIN users ON transactions.id_user = users.id 
        WHERE type IN (3) AND date BETWEEN '$start' AND '$end'
        GROUP BY date, username, amount, keterangan;
    ");
    } else {
        $pemasukan = mysqli_query($db, "
        SELECT date AS tanggal, username AS nama, amount AS jumlah, keterangan AS Keterangan
